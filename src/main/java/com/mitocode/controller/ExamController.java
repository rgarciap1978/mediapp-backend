...
public class ExamController {
    ...
    public ResponseEntity<Void> save(@Valid @RequestBody ExamDTO dto){
        ...
        URI location = ServletUriComponentsBuilder
                .fromCurrentRequest()
                .path("/{id}")
                .buildAndExpand(p.getIdExam())
                .toUri();
        return ResponseEntity.created(location).build();
    }

    @PutMapping
    public ResponseEntity<ExamDTO> update(@Valid @RequestBody ExamDTO dto){
        Exam obj = service.findById(dto.getIdExam());
        if(obj == null) {
            throw new ModelNotFoundException("ID NOT FOUND: " + dto.getIdExam());
        }
        return new ResponseEntity<>(mapper.map(service.update(mapper.map(dto, Exam.class)), ExamDTO.class) , HttpStatus.OK);
    }
